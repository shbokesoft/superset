...
function StickyWrap({
  ...
  if (colWidths && bodyHeight) {
    ...
    const headerColgroup = sticky.hasVerticalScroll && scrollBarSize ? ___EmotionJSX("colgroup", null, colWidths.map((x, i) => // eslint-disable-next-line react/no-array-index-key
    ___EmotionJSX("col", {
      key: i,
      width: x + (i === colWidths.length - 1 ? scrollBarSize : 0)
    }))) : bodyColgroup;
    headerTable = ___EmotionJSX("div", {
      key: "header",
      ref: scrollHeaderRef,
      style: {
        overflow: 'hidden'
    ...
    footerTable = tfoot && ___EmotionJSX("div", {
      key: "footer",
      ref: scrollFooterRef,
      style: {
        overflow: 'hidden'
...
function useInstance(instance) {
  ...
  const useStickyWrap = renderer => {
    ...
    return ___EmotionJSX(StickyWrap, {
      width: width,
      height: height,
      sticky: sticky,
