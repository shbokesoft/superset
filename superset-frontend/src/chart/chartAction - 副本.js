...
const v1ChartDataRequest = async (
  ...
  let parameterStr = window.location.search;
  let parameters = {};
  if (parameterStr) {
    parameterStr = parameterStr.substring(1,parameterStr.length);
    let parameterArray = parameterStr.split("&");
    parameterArray.forEach( (parametercell) => {
      let pid = parametercell.indexOf("=");
      parameters[parametercell.substring(0,pid)] = parametercell.substring(pid+1,parametercell.lenth);
    });
  }
  payload.queries[0].filters.forEach((filterCfg, index) => {
    const match_result = filterCfg.val.match(/\${.*}/g);
    if( match_result ){
      ...
      if (filterVal) {
        ...
        payload.queries[0].filters.splice(index, 1); // 当filterVal值找不到时，删除当前filterItem
      }
    }
  });
  const querySettings = {
